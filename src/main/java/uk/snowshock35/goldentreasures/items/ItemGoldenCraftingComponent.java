...
public class ItemGoldenCraftingComponent extends ItemGoldenTreasures {
    ...
    public IIcon[] icons = new IIcon[ReferencesConfigInfo.GeneralConfigs.NUM_CRAFTING_COMPONENTS];

    public ItemGoldenCraftingComponent() {
        super();
        this.setUnlocalizedName(ReferencesModItems.GOLDEN_CRAFTING_COMPONENT);
    ...
    public void registerIcons(IIconRegister iconRegister) {
        for (int id = 0; id < ReferencesConfigInfo.GeneralConfigs.NUM_CRAFTING_COMPONENTS; id++) {
    ...
    protected IIcon getIconForStack(ItemStack stack) {
        if (stack.hasTagCompound()) {
            ...
            if (id >= ReferencesConfigInfo.GeneralConfigs.NUM_CRAFTING_COMPONENTS)
                id = 0;
