...
public class ItemGoldenStaff extends ItemGoldenTreasuresTogglable {
    ...
    private int getTorchItemMaxCapacity() {
        return ReferencesConfigInfo.GoldenStaff.MAX_CAPACITY_PER_ITEM_TYPE;
    }
    ...
    public void addInformation(ItemStack ist, EntityPlayer player, List list, boolean par4) {
        if (!Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) && !Keyboard.isKeyDown(Keyboard.KEY_RSHIFT))
            return;
        //maps the contents of the Golden staff to a tooltip, so the player can review the torches stored within.
        ...
        if (tagCompound != null) {
            ...
            for (int i = 0; i < tagList.tagCount(); ++i) {
                NBTTagCompound tagItemData = tagList.getCompoundTagAt(i);
                String itemName = tagItemData.getString("Name");
                Item containedItem = ContentHelper.getItem(itemName);
                int quantity = tagItemData.getInteger("Quantity");
            ...
            Item placingItem = ContentHelper.getItem(getTorchPlacementMode(ist));
        ...
        if (blockAttemptingPlacement.canPlaceBlockAt(world, x, y, z)) {
            if (world.canPlaceEntityOnSide(blockAttemptingPlacement, x, y, z, false, side, player, ist)) {
                if (!player.capabilities.isCreativeMode) {
                    ...
                    for (; distance > ReferencesConfigInfo.GoldenStaff.TILE_PER_COST_MULTIPLIER; distance -= ReferencesConfigInfo.GoldenStaff.TILE_PER_COST_MULTIPLIER) {
                        cost++;
                    }
    ...
    public MovingObjectPosition getBlockTarget(World world, EntityPlayer player) {
        ...
        float f7 = f4 * f5;
        float f8 = f3 * f5;
        double d3 = ReferencesConfigInfo.GoldenStaff.MAX_RANGE;
    ...
    public List<Item> getTorchItemList() {
        if (torchItemList == null) {
            synchronized (torchLock) {
                ...
                for (String itemName : ReferencesConfigInfo.GoldenStaff.TORCHES) {
